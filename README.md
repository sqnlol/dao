...
#### Napotkane Błędy (Tydzień 6)

| Problem | Objaw | Status | Rozwiązanie / Plan |
| :--- | :--- | :--- | :--- |
...
| Brak komunikatu przy pustym pliku sugestii | Pusty `suggestions.txt` skutkował brakiem nowych pozycji bez wyjaśnienia. | W toku | Dodanie logu ostrzegawczego + fallback do poprzedniej listy. |
| Duplikaty prób dodania rekordów sprzedaży | Wstawianie tych samych timestampów powodowało IntegrityError (ukryte). | Oczekiwane / Zneutralizowane | Unikalny klucz (nazwa+timestamp+price) w SQLite odrzuca duplikaty; brak dodatkowego działania. |
| Brak timeoutów dla części żądań | Przy słabym łączu wiszące requesty wydłużały operacje. | Planowane | Ustawienie jawnych timeoutów w warstwie HTTP (requests) + retry. |
...
#### Napotkane Błędy (Tydzień 5)

| Problem | Objaw | Status | Rozwiązanie / Plan |
| :--- | :--- | :--- | :--- |
| HTTP 429 przy sekwencyjnym pobraniu historii i ofert | Natychmiastowe pobranie ofert po historii dawało 429. | Rozwiązano | Wydłużenie przerwy (3.0s) + silniejszy backoff (2.0x). |
...
| Brak auto-odświeżenia galerii po pobraniu obrazków | Miniatury wymagały ręcznego przejścia widoku. | Rozwiązano | Dodano końcowy callback przebudowujący siatkę. |
...
| **Powiększone Logo** | Logo w sidebarze zwiększone z 64x64 do 80x80 px dla lepszej widoczności. | Ukończono |
